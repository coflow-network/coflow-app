...
export const makeSessionStore = () => {
  ...
  const signout = async () => {
   ...
   const reset = () => { store.set({state: 'signed-out', data: null} )}
...
export const makeRegistrationStore = () => {
  ...
  const signup = async (email, password) => {
    ...
    if ( !error ) { store.set({state: 'success', data: user}) }
    else { store.set({state: 'error', data: error}); }
  };
  
  const reset = () => { store.set({state: 'idle', data: null} )}
